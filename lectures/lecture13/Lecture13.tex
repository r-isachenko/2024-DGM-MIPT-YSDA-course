...
\begin{frame}{Flow Matching}
	Let consider ODE dynamic in time interval $t \in [0, 1]$ with boundaries $\bx(0) \sim p(\bx)$, $\bx(1) \sim \pi(\bx)$. Here $p(\bx)$ is a base distribution ($\cN(0, \bI)$) and $\pi(\bx)$ is a true data distribution.
	\[
		d \bx = \bu (\bx, t) dt
	\]
	$\bu (\bx, t): \bbR^m \times [0, 1] \rightarrow \bbR^m$ is a vector field.
	\[
		\frac{\partial p(\bx, t)}{\partial t} = - \text{div}\left(\bu(\bx, t) p(\bx, t)\right) 
	\]
	
	If we know the true vector field $\bu (\bx, t)$, then KFP equation gives us the way to compute the density $p(\bx, t)$.
	\begin{block}{Flow Matching}
		\[
			\bbE_{t \sim U[0, 1]} \bbE_{\bx \sim p(\bx, t)}\| \bu(\bx, t) - \bu_{\btheta}(\bx, t) \|^2 \rightarrow \min_{\btheta}
		\]
	\end{block}
