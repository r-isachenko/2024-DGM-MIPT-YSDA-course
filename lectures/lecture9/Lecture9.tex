...
\begin{frame}{Summary}
	\begin{itemize}
		...
		\vfill
		\item Reverse process allows to recover the samples from the real distribution $\pi(\bx)$ from the noise, but it is intractable.
		\vfill
		...
		\vfill
		\item At each step DDPM predicts the noise that was used in the forward diffusion process. This noise is used in the reverse process.
	\end{itemize}
\end{frame}
%=======
\end{document} 